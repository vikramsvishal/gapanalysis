#!/usr/bin/env python3
"""Unified Network and Server Hardware Catalog Governance GUI - V6
...
APP_TITLE = "Unified Network and Server Hardware Governance V6"
...
def match_server_model(manufacturer: Any, model_id: Any, indexes: ServerCatalogIndexes) -> CatalogMatch:
    manufacturer_value, model_value = text(manufacturer), text(model_id)
    manufacturer_key, model_key = value_key(manufacturer_value), value_key(model_value)
    if not model_key:
        return CatalogMatch(None, "NO MATCH", "BLANK MODEL ID")
    if manufacturer_key:
        ids = indexes.manufacturer_model_index.get(f"{manufacturer_key}|{model_key}", ())
        if ids:
            return choose_unique([indexes.records[i] for i in ids], "EXACT MATCH", "EXACT MANUFACTURER + MODEL ID", 10000)
    ids = indexes.model_only_index.get(model_key, ())
    if not ids:
        return CatalogMatch(None, "NO MATCH", "MODEL ID NOT FOUND IN SERVER CATALOG")
    records = [indexes.records[i] for i in ids]
    if len(records) == 1:
        detail = "MODEL ID ONLY - MANUFACTURER REVIEW" if manufacturer_key and records[0].manufacturer_key != manufacturer_key else "EXACT MODEL ID"
        return CatalogMatch(records[0], "PARTIAL MATCH" if "REVIEW" in detail else "EXACT MATCH", detail, 8500)
    # If several rows share a model, manufacturer cannot be ignored safely.
    return choose_unique(records, "PARTIAL MATCH", "MODEL ID ONLY", 8000)
...
def run_network(catalog_path: Path, cmdb_path: Path, category_path: Path, output_dir: Path) -> dict[str, Any]:
    ...
    write_table_workbook(output_dir / "NW_Catalog_Match.xlsx", "NW Catalog Match", match_headers, match_rows, date_headers)
    make_load_workbook(output_dir / "NW_Category_Load.xlsx", prefix_headers, category, load_rows)
    write_table_workbook(output_dir / "NW_Category_Catalog_Drift.xlsx", "NW Category Drift", drift_headers, drift_rows)
    write_table_workbook(output_dir / "NW_Catalog_Enhancement_Recommendations.xlsx", "NW Catalog Recommendations", rec_headers, recommendation_rows)
    return dict(stats)
...
def run_server(catalog_path: Path, cmdb_path: Path, category_path: Path, output_dir: Path) -> dict[str, Any]:
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    date_headers = {"Catalog End of Life Date", "Catalog End of Support Date"}
    write_table_workbook(output_dir / "Server_Catalog_Match.xlsx", "Server Catalog Match", match_headers, match_rows, date_headers)
    make_load_workbook(output_dir / "Server_Category_Load.xlsx", prefix_headers, category, load_rows)
    write_table_workbook(output_dir / "Server_Category_Catalog_Drift.xlsx", "Server Category Drift", drift_headers, drift_rows)
    write_table_workbook(output_dir / "Server_Catalog_Enhancement_Recommendations.xlsx", "Server Catalog Recommendations", rec_headers, recommendation_rows)
    return dict(stats)
    ...
    class GovernanceApp(tk.Tk):
        ...
        def execute(self, mode: str) -> None:
            ...
            try:
                ...
                write_governance_summary(output / "Governance_Summary.xlsx", network_stats, server_stats)
                parts = []
                if network_stats:
                    parts.append(
