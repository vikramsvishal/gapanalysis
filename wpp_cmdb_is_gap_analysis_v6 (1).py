"""WPP Enterprise CMDB and Inventory Services Gap Analysis V6
...
import os, re, json, sqlite3, hashlib, ipaddress, traceback, threading, sys, subprocess, importlib
from pathlib import Path
from datetime import datetime, date
from typing import Any, Dict, List, Optional, Tuple

def _ensure_runtime_dependency(import_name: str, pip_name: str):
    try:
        ...
        try:
            ...
            raise RuntimeError(f"Missing dependency {pip_name}. Automatic installation failed: {exc}") from exc

pd = _ensure_runtime_dependency("pandas", "pandas")
_ensure_runtime_dependency("openpyxl", "openpyxl")
...
APP_NAME = "WPP Enterprise CMDB and Inventory Services Gap Analysis V6"
APP_DIR = Path(__file__).resolve().parent
REGISTRY_PATH = APP_DIR / "v6_resource_registry.json"
LEARNING_DB = APP_DIR / "v6_flag_intelligence.sqlite"
...
SESSION_RESOURCES = ["IS Operating System Report", "NW CMDB Report", "Server CMDB Report"]
...
# V6 treats versions as supported until extended support end. Refresh through code/config
...
def read_tabular(path: str, keep_metadata: bool=False):
    ...
    if ext == ".csv":
        ...
        _ensure_runtime_dependency("pyxlsb", "pyxlsb")
...
def reconcile_server(cm,isr,cat,fi,progress=lambda m,p:None):
    ...
    if not support and not managed:
        raise ValueError("Server CMDB requires Support Group or Managed By Group for W-KYN scoping")
    ...
    scoped=cm[scope].copy().reset_index(drop=True);outside=cm[~scope].copy()
    if scoped.empty:
        return pd.DataFrame(),outside,pd.DataFrame(),pd.DataFrame()
    name=fi.field("hostname","Server CMDB Report",scoped);lc=fi.field("os_lifecycle_status","Server CMDB Report",scoped);ip=fi.field("ip_address","Server CMDB Report",scoped);serial=fi.field("os_parent_serial_number","Server CMDB Report",scoped,False);osc=fi.field("operating_system_name","Server CMDB Report",scoped);osv=fi.field("operating_system_version","Server CMDB Report",scoped);fq=fi.field("fully_qualified_hostname","Server CMDB Report",scoped,False);cl=fi.field("class","Server CMDB Report",scoped,False);category=fi.field("category","Server CMDB Report",scoped,False);subcat=fi.field("subcategory","Server CMDB Report",scoped,False)
    attrs=find_col(scoped,"Attribute Field Value","Attributes",required=False)
    lstatus=find_col(scoped,"Life Cycle Stage Status",required=False);manufacturer=find_col(scoped,"Manufacturer",required=False)
    base=pd.DataFrame({
        "Configuration Item":scoped[name],"Class":scoped[cl] if cl else "Server",
        "Life Cycle Stage":scoped[lc],"Life Cycle Stage Status":scoped[lstatus] if lstatus else "",
        "Manufacturer":scoped[manufacturer] if manufacturer else "","Model ID":"","Model.Name":"","Model number":"",
        "Serial number":scoped[serial] if serial else "","IP Address":scoped[ip],
        "Firmware version":scoped[osc].map(clean)+" "+scoped[osv].map(clean),
        "Fully qualified domain name":scoped[fq] if fq else scoped[name],
        "category":scoped[category] if category else "Server","subcategory":scoped[subcat] if subcat else "Server",
        "Attribute Field Value":scoped[attrs] if attrs else ""})
    ...
    progress("Server reconciliation complete",100)
    return result,outside,duplicates,historical

def server_summary(r, outside, duplicates, historical):
    if r is None or r.empty:
        return pd.DataFrame([("Server CMDB records analyzed",0),("Out of Kyndryl scope",len(outside)),("CMDB duplicate records",len(duplicates)),("Historical EOL suppressed",len(historical))],columns=["Metric","Value"])
    return pd.DataFrame([
        ("Server CMDB records analyzed",len(r)),
        ("Records present in IS",int(r["Present in IS ?"].astype(str).str.startswith("Yes").sum())),
        ("Records not present in IS",int((r["Present in IS ?"]=="No").sum())),
        ("Catalogue exact matches",int((r["Catalogue Match?"]=="Match").sum())),
        ("Catalogue closest matches proposed",int((r["Catalogue Match?"]=="Closest Match Proposed").sum())),
        ("Catalogue updates required",int(r["Action"].str.contains("Catalogue update required",na=False).sum())),
        ("Inventory OS updates required",int(r["Action"].str.contains("Inventory OS update required",na=False).sum())),
        ("Load To IS records",int(r["Action"].str.contains("Load To IS",na=False).sum())),
        ("Retire From IS records",int(r["Action"].str.contains("Retire From IS",na=False).sum())),
        ("Out of Kyndryl scope",len(outside)),("CMDB duplicate records",len(duplicates)),("Historical EOL suppressed",len(historical))],columns=["Metric","Value"])
...
class FlagLearning:
    ...
    def __init__(self,path=None):
        self.conn=sqlite3.connect(path or LEARNING_DB)
        self.conn.executescript("CREATE TABLE IF NOT EXISTS reports(hash TEXT PRIMARY KEY,loaded TEXT,name TEXT);CREATE TABLE IF NOT EXISTS observations(field TEXT,value TEXT,priority TEXT,cohort TEXT);CREATE TABLE IF NOT EXISTS anomalies(report_hash TEXT,os_opaque_id TEXT,hostname TEXT,field TEXT,value TEXT,type TEXT);")
        self.conn.commit()
    def ingest(self,path):
        ...
        oid=find_col(df,"os_opaque_id","OS Opaque ID",required=False);host=find_col(df,"hostname","host_name",required=False)
        duplicates=set(df.loc[df[oid].duplicated(False),oid])-{""} if oid else set()
        flags=[]
        for c in df.columns:
        ...
        for _,r in df.iterrows():
            record_id=clean(r.get(oid,"")) if oid else "";hostname=clean(r.get(host,"")) if host else ""
            if not record_id:self.conn.execute("INSERT INTO anomalies VALUES(?,?,?,?,?,?)",(digest,record_id,hostname,"os_opaque_id","","Missing unique identifier"))
            if record_id in duplicates:self.conn.execute("INSERT INTO anomalies VALUES(?,?,?,?,?,?)",(digest,record_id,hostname,"os_opaque_id",record_id,"Duplicate os_opaque_id"))
            cohort="|".join([pkey(r.get(x,"")) for x in self.COHORT_FIELDS])
            for field,priority in flags:
                raw=clean(r[field]);value=raw.upper()
                if raw.lower() in self.PLACEHOLDERS:
                    self.conn.execute("INSERT INTO anomalies VALUES(?,?,?,?,?,?)",(digest,record_id,hostname,field,raw,"Placeholder value"));continue
    ...
    def predict(self,field,priority,cohort):
        ...
        for n in range(len(parts),0,-1):
            ...
            if vals:
                y=vals.count("Y");no=vals.count("N");support=y+no;agreement=max(y,no)/support;value="Y" if y>=no else "N"
                return (value,agreement,support,"Auto-prefill",n) if agreement>=.75 else ("",agreement,support,"No proposal",n)
        return "",0,0,"No evidence",0
    def anomalies(self,report_hash):
        return pd.read_sql_query("SELECT os_opaque_id,hostname,field,value,type FROM anomalies WHERE report_hash=?",self.conn,params=(report_hash,))
...
def generate_bulk_load(rec,source_type,is_report,template,outdir,fmt,askfq,askcat,progress=lambda m,p:None):
    ...
    if missing.any():
        if source_type=="network" and askfq(int(missing.sum())):fq.loc[missing]=candidates.loc[missing,"Configuration Item"].map(lambda x:normalize_hostname(x).lower()+".nw.wpp.net")
    ...
    flag_info=[]
    if meta is not None:
        isdf,ismeta=read_tabular(is_report,True)
        ismeta=ismeta if ismeta is not None else pd.Series("",index=isdf.columns)
        for h in headers:
            source_col=next((c for c in isdf.columns if hkey(c)==hkey(h)),None)
            if source_col:
                values={clean(x).upper() for x in isdf[source_col] if clean(x)}
                if values and values.issubset({"Y","N"}):flag_info.append((h,"Priority 1" if clean(ismeta.get(source_col,"")).lower()=="mandatory" else "Priority 2"))
    for ix,r in candidates.iterrows():
        ...
        cohort="|".join([pkey(context["subcategory"]),pkey(context["parent_system_type"]),pkey(context["image_purpose"]),pkey("Production"),pkey(context["os_name"]),pkey(r.get("business_criticality_device_grouping","")),pkey(r.get("os_inventory_item_owned_by","")),pkey(r.get("os_inventory_item_managed_by",""))])
...
FEATURE_MANIFEST={
 "NW_RECONCILIATION":{"version":"6.0","engine":"reconcile_nw"},
 "SERVER_RECONCILIATION":{"version":"6.0","engine":"reconcile_server"},
 "NW_OS_BULK_LOAD":{"version":"6.0","engine":"generate_bulk_load"},
 "SERVER_OS_BULK_LOAD":{"version":"6.0","engine":"generate_bulk_load"},
 "FIELD_INTELLIGENCE":{"version":"6.0","engine":"FieldIntelligence"},
 "FLAG_INTELLIGENCE":{"version":"6.0","engine":"FlagLearning"}}
...
class ReconciliationAgent:
    def execute(self,source_type,cmdb,is_report,catalog,field_mapping=None):
        ...
        if source_type=="network":return {"reconciliation":reconcile_nw(read_tabular(cmdb),read_tabular(is_report),read_tabular(catalog),fi)}
        r,outside,dups,historical=reconcile_server(read_tabular(cmdb),read_tabular(is_report),read_tabular(catalog),fi)
        return {"reconciliation":r,"out_of_scope":outside,"duplicates":dups,"historical_eol_suppressed":historical}

class BulkLoadAgent:
    def execute(self,reconciliation,source_type,is_report,template,output_dir,fmt="xlsx",fqdn_decision=lambda n:False,catalog_decision=lambda n:True):
        return generate_bulk_load(reconciliation,source_type,is_report,template,output_dir,fmt,fqdn_decision,catalog_decision)

class ApplicationService:
    def reconcile(self,request):return ReconciliationAgent().execute(**request)
    def generate_bulk_load(self,request):return BulkLoadAgent().execute(**request)
...
class App(tk.Tk):
    ...
    def reconcile_nw_click(self):
        ...
        if not out:return
        def task():
            self._load_field_mapping();r=reconcile_nw(read_tabular(self.paths["NW CMDB Report"]),read_tabular(self.paths["IS Operating System Report"]),read_tabular(self.paths["IS Product Catalog Operating System"]),self.fi,self.progress);self.results["network"]=r;p=next_versioned_path(out,"NW CMDB to IS Reconciliation V6","xlsx");write_workbook(p,{"Summary":nw_summary(r),"Reconciliation":r,"Load To IS":r[r.Action.str.contains("Load To IS",na=False)],"Retire From IS":r[r.Action.str.contains("Retire From IS",na=False)],"Inventory OS Update Required":r[r.Action.str.contains("Inventory OS update required",na=False)],"Catalogue Update Required":r[r.Action.str.contains("Catalogue update required",na=False)]});self.say("Created: "+p);self.after(0,lambda path=p:messagebox.showinfo("Complete",path))
        self.execute(task)
    def reconcile_server_click(self):
        ...
        def task():
            ...
            r,outside,dups,historical=reconcile_server(read_tabular(self.paths["Server CMDB Report"]),read_tabular(self.paths["IS Operating System Report"]),read_tabular(self.paths["IS Product Catalog Operating System"]),self.fi,self.progress)
            self.results["server"]=r
            p=next_versioned_path(out,"Server CMDB to IS Reconciliation V6","xlsx")
            write_workbook(p,{"Summary":server_summary(r,outside,dups,historical),"Reconciliation":r,"Load To IS":r[r.Action.str.contains("Load To IS",na=False)] if not r.empty else r,"Retire From IS":r[r.Action.str.contains("Retire From IS",na=False)] if not r.empty else r,"Inventory OS Update Required":r[r.Action.str.contains("Inventory OS update required",na=False)] if not r.empty else r,"Catalogue Update Required":r[r.Action.str.contains("Catalogue update required",na=False)] if not r.empty else r,"Out of Kyndryl Scope":outside,"Historical EOL Suppressed":historical,"CMDB Duplicate Review":dups})
            self.registry.mark_used("catalog_os");self.say("Created: "+p);self.after(0,lambda path=p:messagebox.showinfo("Complete",path))
        self.execute(task)
    def _resource_notice(self):
        items=[]
        for rid,label in [("bulk_load_template","Bulk Load Template"),("catalog_os","OS Product Catalogue")]:
            item=self.registry.get(rid)
            if not item:return False
            loaded=datetime.fromisoformat(item["loaded_on"]).strftime("%d.%b.%Y").upper()
            items.append(f"{label}: {item['file_name']} (loaded {loaded})")
        text="Current persistent resources:\n\n"+"\n".join(items)+"\n\nThe bulk load will be created using these files. If either resource has changed, upload the new file before continuing."
        return messagebox.askyesno("Current Bulk Load Resources",text)
    def bulk_click(self,source):
        ...
        if not self._resource_notice():return
        out=self.output_dir();
        if not out:return
...
if __name__=="__main__":
    try:App().mainloop()
        ...
        print("V6 startup failed:",exc,file=sys.stderr)
        traceback.print_exc()
        raise
