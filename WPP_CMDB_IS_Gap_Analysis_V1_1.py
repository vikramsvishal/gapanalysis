"""WPP Enterprise CMDB and Inventory Services Gap Analysis V1.1
...
APP_NAME = "WPP Enterprise CMDB and Inventory Services Gap Analysis V1.1"
APP_DIR = Path(__file__).resolve().parent
REGISTRY_PATH = APP_DIR / "v1_1_resource_registry.json"
LEARNING_DB = APP_DIR / "v1_1_flag_intelligence.sqlite"
...
def vkey(v: Any) -> str:
    """Return a canonical software/firmware version key.

    Numeric components are converted to integers so zero-padded values compare
    equally, while alphabetic release suffixes remain significant.
    """
    raw = clean(v).lower()
    if not raw:
        return ""
    raw = raw.replace("–", "-").replace("—", "-")
    raw = re.sub(r"^(?:version|ver|release|rel|v)\s*[:=_-]?\s*", "", raw)
    raw = re.sub(r"\s+", "", raw)

    # Normalize every numeric run independently: 07 -> 7, 001 -> 1.
    raw = re.sub(r"\d+", lambda match: str(int(match.group(0))), raw)

    # Normalize punctuation without removing meaningful alpha suffixes such as a/b.
    raw = raw.replace("_", ".")
    raw = re.sub(r"[\s/]+", ".", raw)
...
def _format_excel_workbook(path: str) -> None:
    ...
    for worksheet in workbook.worksheets:
        ...
        # Reconciliation summary action color coding.
        if worksheet.title.lower() == "summary" and worksheet.max_column >= 2:
            metric_column = None
            for column_index in range(1, worksheet.max_column + 1):
                if pkey(worksheet.cell(1, column_index).value) == "metric":
                    metric_column = column_index
                    break
            if metric_column:
                load_fill = PatternFill("solid", fgColor="E2F0D9")
                retire_fill = PatternFill("solid", fgColor="F4CCCC")
                for row_index in range(2, worksheet.max_row + 1):
                    metric = pkey(worksheet.cell(row_index, metric_column).value)
                    if "loadtoisrecords" in metric:
                        worksheet.cell(row_index, metric_column).fill = load_fill
                    elif "retirefromisrecords" in metric:
                        worksheet.cell(row_index, metric_column).fill = retire_fill
...
def catalog_index(cat: pd.DataFrame, fi: FieldIntelligence):
    """Build exact and suffix-tolerant OS catalogue indexes using canonical keys."""
    product_col = fi.field("operating_system_name", "IS Product Catalog Operating System", cat)
    version_col = fi.field("operating_system_version", "IS Product Catalog Operating System", cat)
    provider_col = fi.field("operating_system_provider", "IS Product Catalog Operating System", cat, False)

    exact: Dict[str, Dict[str, str]] = {}
    base: Dict[str, Dict[str, str]] = {}
    for _, row in cat.iterrows():
        product = {
            "name": clean(row[product_col]),
            "version": clean(row[version_col]),
            "provider": clean(row[provider_col]) if provider_col else "",
        }
        name_key = pkey(product["name"])
        version_key = vkey(product["version"])
        if not name_key or not version_key:
            continue

        # Exact canonical index resolves zero padding, case, spaces and punctuation.
        exact.setdefault(name_key + "|" + version_key, product)

        # Secondary index allows only a terminal alphabetic build suffix to differ.
        # It does not drop numeric version components.
        suffix_tolerant = re.sub(r"(?<=\d)[a-z]+$", "", version_key)
        base.setdefault(name_key + "|" + suffix_tolerant, product)
    return exact, base
...
def reconcile_server(cm, isr, cat, fi, progress=lambda m,p:None):
    ...
    duplicates=base[base.duplicated(["_name_key","_life_key"],False)].copy()
    base=base.drop_duplicates(["_name_key","_life_key"],keep="first")
    active=set(base.loc[base["_life_key"]=="operational","_name_key"])
    historical=base[(base["_life_key"]=="end of life") & base["_name_key"].isin(active)].copy()
    eligible=base[~((base["_life_key"]=="end of life") & base["_name_key"].isin(active))].copy()
    context=eligible[["Configuration Item","category","subcategory","Attribute Field Value"]].reset_index(drop=True)
    standardized=eligible.drop(columns=["_name_key","_life_key","category","subcategory","Attribute Field Value"])
    result=reconcile_nw(standardized,isr,cat,fi,progress)
...
def server_summary(r,outside,duplicates,historical):
    vals=[("Server CMDB records analyzed",len(r)),("Records present in IS",int(r["Present in IS ?"].astype(str).str.startswith("Yes").sum()) if not r.empty else 0),("Records not present in IS",int((r["Present in IS ?"]=="No").sum()) if not r.empty else 0),("Load To IS records",int(r["Action"].str.contains("Load To IS",na=False).sum()) if not r.empty else 0),("Retire From IS records",int(r["Action"].str.contains("Retire From IS",na=False).sum()) if not r.empty else 0),("Out of W-KYN scope",len(outside)),("CMDB duplicate records",len(duplicates)),("Historical EOL suppressed",len(historical))]
    return pd.DataFrame(vals,columns=["Metric","Value"])
...
def generate_bulk_load(rec,source_type,is_report,template,outdir,fmt,askfq,askcat,progress=lambda m,p:None):
    ...
    learner=FlagLearning();report_hash=learner.ingest(is_report)
    flag_info=[]
    if meta is not None:
    ...
    learning_anomalies=learner.anomalies(report_hash);learner.close();metadata=["Reference"]*3+([clean(meta[h]) for h in headers] if meta is not None else [""]*len(headers));final=pd.concat([pd.DataFrame([metadata],columns=extras+headers),pd.DataFrame(rows,columns=extras+headers)],ignore_index=True)
    label="NW - OS Bulk Load" if source_type=="network" else "Server - OS Bulk Load";load_path=next_versioned_path(outdir,label,fmt);_write_formatted_load(final,load_path) if fmt=="xlsx" else final.to_csv(load_path,index=False)
    validation=next_versioned_path(outdir,label+" Validation","xlsx");write_workbook(validation,{"Policy Intelligence":pd.DataFrame(policy_audit),"Flag Intelligence":pd.DataFrame(learning_audit),"IP Validation":pd.DataFrame(ip_audit),"CMDB Correction Required":duplicate,"Missing FQDN Excluded":fq_excluded,"IS Learning Anomalies":learning_anomalies})
...
def run_hardware_governance(domain,cmdb_path,category_path,catalog_path,outdir,progress=lambda m,p:None,os_reconciliation=None,is_os_path=None):
    progress("Loading hardware governance inputs",10);cm=read_tabular(cmdb_path);catdf=read_tabular(category_path);catalog=read_tabular(catalog_path);idx=_hw_index(catalog)
    ...
    cserial=find_col(catdf,"network_serial_number" if domain=="network" else "serial_number","server_serial_number" if domain=="server" else "network_serial_number");cmap=defaultdict(list)
    for i,r in catdf.iterrows():
        if serial_key(r[cserial]):cmap[serial_key(r[cserial])].append(i)
    matchrows=[];loadrows=[];drift=[];recs=[];hostname_review=[];retire_review=[];actions=[]
    recon_by_serial={}
    if os_reconciliation is not None and not os_reconciliation.empty:
        for _,rr in os_reconciliation.iterrows():
            key=serial_key(rr.get("Serial number",""))
            if key:recon_by_serial[key]=rr
    is_os_by_serial=defaultdict(list)
    if is_os_path:
        ios=read_tabular(is_os_path);ios_serial=find_col(ios,"os_parent_serial_number",required=False);ios_host=find_col(ios,"hostname",required=False)
        if ios_serial:
            for _,ior in ios.iterrows():
                key=serial_key(ior[ios_serial])
                if key:is_os_by_serial[key].append(ior)
    prefix=["CMDB Hostname","CMDB Serial Number","CMDB Model ID"]+(["CMDB Model Number"] if domain=="network" else [])+["CMDB Lifecycle Stage","CMDB Lifecycle Stage Status"]
    for _,r in cm.iterrows():
        hm=hardware_match(domain,r[man],r[model],idx);sk=serial_key(r[serial]);presence="FOUND IN CATEGORY" if len(cmap.get(sk,[]))==1 else "FOUND - DUPLICATE NORMALIZED SERIAL" if len(cmap.get(sk,[]))>1 else "MISSING FROM CATEGORY"
        rr=recon_by_serial.get(sk);os_action=clean(rr.get("Action","")) if rr is not None else "";is_load="Load To IS" in os_action;existing_os=is_os_by_serial.get(sk,[])
        recommended="NO LOAD ACTION"
        if is_load and presence=="FOUND IN CATEGORY" and not existing_os:recommended="LOAD OS ONLY"
        elif is_load and presence=="MISSING FROM CATEGORY":recommended="LOAD CATEGORY AND OS"
        elif presence=="FOUND - DUPLICATE NORMALIZED SERIAL":recommended="CATEGORY DATA QUALITY REVIEW - DUPLICATE NORMALIZED SERIAL"
        if existing_os and presence=="FOUND IN CATEGORY":
            for ior in existing_os:
                ih=clean(ior[ios_host]) if ios_host else ""
                if pkey(clean(r[host]).split('.')[0])!=pkey(ih.split('.')[0]):hostname_review.append({"Serial Number":clean(r[serial]),"CMDB Hostname":clean(r[host]),"IS OS Hostname":ih,"Status":"HOSTNAME CHANGED","Review":"REVIEW HARDWARE REUSE / HOSTNAME CHANGE"})
        row={"CMDB Hostname":clean(r[host]),"CMDB Serial Number":clean(r[serial]),"CMDB Manufacturer":clean(r[man]),"CMDB Model":clean(r[model]),"CMDB Lifecycle Stage":clean(r[life]),"Catalog Match Status":hm.status,"Catalog Match Detail":hm.detail,"Match Score":hm.score,"Category Presence Status":presence,"OS Reconciliation Action":os_action,"Recommended Action":recommended}
        if hm.record:row.update({"Catalog Manufacturer":hm.record["manufacturer"],"Catalog Hardware Type":hm.record["hardware_type"],"Catalog Hardware Model":hm.record["hardware_model"],"Catalog Subcategory":hm.record["subcategory"],"Catalog EOL":hm.record["eol"],"Catalog EOSL":hm.record["eosl"]})
        matchrows.append(row)
        if lifecycle(r[life])=="operational" and recommended=="LOAD CATEGORY AND OS":
            vals={c:"" for c in catdf.columns};pref="network_" if domain=="network" else "server_";vals[cserial]=clean(r[serial]);
            for f in ["id",pref+"opaque_id",pref+"catalog_source"]:
                if f in vals:vals[f]=""
            ...
            if hm.status=="EXACT MATCH" and hm.record:
                ...
                remarks=next((c for c in catdf.columns if hkey(c) in {hkey(x) for x in ["comments","notes","remarks","description"]}),None)
                if remarks:vals[remarks]="CATALOG ENHANCEMENT REQUIRED"
            pre=[clean(r[host]),clean(r[serial]),clean(r[mid]) if mid else ""]+([clean(r[model])] if domain=="network" else [])+[clean(r[life]),clean(r[lstat]) if lstat else ""];loadrows.append(pre+[vals[c] for c in catdf.columns])
        if hm.status!="EXACT MATCH":recs.append(row)
        actions.append({"CMDB Hostname":clean(r[host]),"CMDB Serial Number":clean(r[serial]),"Recommended Action":recommended})
    if is_os_path:
        for sk,ids in cmap.items():
            if sk in is_os_by_serial and not any(serial_key(x[serial])==sk for _,x in cm.iterrows()):
                retire_review.append({"Normalized Serial":sk,"Status":"REVIEW REQUIRED - CATEGORY AND IS OS SERIAL NOT FOUND IN CMDB","Recommended Action":"POTENTIAL RETIRE FROM IS CANDIDATE"})
    label="NW" if domain=="network" else "Server";paths=[]
    p=next_versioned_path(outdir,label+" Hardware Catalog Match","xlsx");write_workbook(p,{"Catalog Match":pd.DataFrame(matchrows)});paths.append(p)
    p=next_versioned_path(outdir,label+" Category Load","xlsx");metadata=["Reference"]*len(prefix)+[""]*len(catdf.columns);_write_formatted_load(pd.DataFrame([metadata]+loadrows,columns=prefix+list(catdf.columns)),p);paths.append(p)
    p=next_versioned_path(outdir,label+" Catalog Enhancement Recommendations","xlsx");write_workbook(p,{"Recommendations":pd.DataFrame(recs)});paths.append(p)
    p=next_versioned_path(outdir,label+" Hardware Governance Review","xlsx");write_workbook(p,{"Actions":pd.DataFrame(actions),"Hostname Changed Review":pd.DataFrame(hostname_review),"Potential Retire Candidates":pd.DataFrame(retire_review)});paths.append(p)
    progress("Hardware governance complete",100);return paths
...
class App(tk.Tk):
    ...
    def _build(self):
        ...
        for text,cmd,desc in [("Run NW HW Catalog and IS Category Match",lambda:self.hardware_click("network"),"Creates NW catalogue match, Category load and recommendations"),("Run Server HW Catalog and IS Category Match",lambda:self.hardware_click("server"),"Creates Server catalogue match, Category load and recommendations")]:
    ...
    def clear(self,name):
        ...
        if name in PERSISTENT_RESOURCES:self.registry.remove(PERSISTENT_RESOURCES[name])
    ...
    def hardware_click(self,source):
        ...
        def task():
            if source not in self.results:
                if not self.require("IS Operating System Report","IS Product Catalog Operating System"):return
                if source=="network":self.results[source]=reconcile_nw(read_tabular(self.paths[cmdb]),read_tabular(self.paths["IS Operating System Report"]),read_tabular(self.paths["IS Product Catalog Operating System"]),self.fi,self.progress)
                else:self.results[source]=reconcile_server(read_tabular(self.paths[cmdb]),read_tabular(self.paths["IS Operating System Report"]),read_tabular(self.paths["IS Product Catalog Operating System"]),self.fi,self.progress)[0]
            paths=run_hardware_governance(source,self.paths[cmdb],self.paths[category],self.paths[catalog],out,self.progress,self.results[source],self.paths.get("IS Operating System Report"))
            for p in paths:self.say("Created: "+p)
            self.after(0,lambda outputs=list(paths):messagebox.showinfo("Hardware Governance Complete","\n".join(outputs)))
        self.execute(task)
    def reconcile_nw_click(self):
        ...
        if not out:return
        def task():
            self._load_field_mapping();r=reconcile_nw(read_tabular(self.paths["NW CMDB Report"]),read_tabular(self.paths["IS Operating System Report"]),read_tabular(self.paths["IS Product Catalog Operating System"]),self.fi,self.progress);self.results["network"]=r;p=next_versioned_path(out,"NW CMDB to IS Reconciliation","xlsx");write_workbook(p,{"Summary":nw_summary(r),"Reconciliation":r,"Load To IS":r[r.Action.str.contains("Load To IS",na=False)],"Retire From IS":r[r.Action.str.contains("Retire From IS",na=False)],"Inventory OS Update Required":r[r.Action.str.contains("Inventory OS update required",na=False)],"Catalogue Update Required":r[r.Action.str.contains("Catalogue update required",na=False)]});self.say("Created: "+p);self.after(0,lambda path=p:messagebox.showinfo("Complete",path))
        self.execute(task)
    def reconcile_server_click(self):
        ...
        if not out:return
        def task():
            self._load_field_mapping();r,outside,dups,historical=reconcile_server(read_tabular(self.paths["Server CMDB Report"]),read_tabular(self.paths["IS Operating System Report"]),read_tabular(self.paths["IS Product Catalog Operating System"]),self.fi,self.progress);self.results["server"]=r;p=next_versioned_path(out,"Server CMDB to IS Reconciliation","xlsx");write_workbook(p,{"Summary":server_summary(r,outside,dups,historical),"Reconciliation":r,"Load To IS":r[r.Action.str.contains("Load To IS",na=False)] if not r.empty else r,"Retire From IS":r[r.Action.str.contains("Retire From IS",na=False)] if not r.empty else r,"Out of W-KYN Scope":outside,"Historical EOL Suppressed":historical,"CMDB Duplicate Review":dups});self.say("Created: "+p);self.after(0,lambda path=p:messagebox.showinfo("Complete",path))
        self.execute(task)
    ...
    def bulk_click(self,source):
        ...
        def task():
            if source not in self.results:
                ...
                else:r,_,_,_=reconcile_server(read_tabular(self.paths["Server CMDB Report"]),read_tabular(self.paths["IS Operating System Report"]),read_tabular(self.paths["IS Product Catalog Operating System"]),self.fi,self.progress)
            ...
            for p in paths:self.say("Created: "+p)
            self.after(0,lambda outputs=list(paths):messagebox.showinfo("Complete","\n".join(outputs)))
        self.execute(task)
