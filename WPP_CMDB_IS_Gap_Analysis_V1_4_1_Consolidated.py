"""WPP Enterprise CMDB and Inventory Services Gap Analysis V1.4.1
...
APP_NAME = "WPP Enterprise CMDB and Inventory Services Gap Analysis V1.4.1"
APP_DIR = Path(__file__).resolve().parent
REGISTRY_PATH = APP_DIR / "v1_4_1_resource_registry.json"
LEARNING_DB = APP_DIR / "v1_4_1_flag_intelligence.sqlite"
...
def generate_bulk_load(rec,source_type,is_report,template,outdir,fmt,askfq,askcat,progress=lambda m,p:None,governance=None):
    label="NW" if source_type=="network" else "Server"
    progress(f"Starting {label} OS Bulk Load workflow",2)
    ...
    candidates["Candidate Set ID"]=candidates.apply(lambda r:candidate_set_id(source_type,r),axis=1)
    progress(f"Using {label} reconciliation Load To IS population: {len(candidates)}",8)
    decisions=governance["decisions"] if governance else pd.DataFrame()
    decmap={x["Candidate Set ID"]:x for _,x in decisions.iterrows()}
    consistency=[];eligible=[]
    for _,r in candidates.iterrows():
        cid=r["Candidate Set ID"];d=decmap.get(cid);ok=d is not None and d["Recommended Action"] in {"LOAD OS ONLY","LOAD CATEGORY AND OS"}
        consistency.append({"Domain":label,"Candidate Set ID":cid,"CMDB Hostname":r["Configuration Item"],"CMDB Serial Number":r["Serial number"],"In Reconciliation Load To IS":"Y","In Hardware Governance":"Y" if d is not None else "N","In Category Load":"Y" if d is not None and d["Recommended Action"]=="LOAD CATEGORY AND OS" else "N","In OS Bulk Load":"Y" if ok else "N","Consistency Status":"PASS" if ok else "BLOCKED","Blocking Reason":"" if ok else "MISSING OR BLOCKED HARDWARE/CATEGORY DECISION"})
        if ok:eligible.append((r,d))
    progress("Candidate consistency validation complete",15)

    template_df,meta=safe_template(template);headers=list(template_df.columns)
    extras=["CMDB Hostname","CMDB Serial Number","CMDB Lifecycle Stage","Reconciliation Action","Candidate Set ID","Category Load Required","Parent Dependency Status","Hardware Catalog Match Status"]
    header_lookup={hkey(h):h for h in headers}
    policy_aliases={
        "is_disaster_recovery_contracted":["is_disaster_recovery_contracted","is disaster recovery contracted"],
        "is_designated_for_compliance_reporting":["is_designated_for_compliance_reporting","is designated for compliance reporting","is_designated_compliance_reporting_required","is designated compliance reporting required","is compliance reporting required"],
        "bac_id":["bac_id","bac id"],
        "bam_id":["bam_id","bam id"],
        "is_anti_virus_required":["is_anti_virus_required","is anti virus required","is antivirus required"],
        "is_patching_required":["is_patching_required","is patching required"],
        "is_health_check_required":["is_health_check_required","is health check required"],
        "health_check_interval":["health_check_interval","health check interval"],
        "health_check_unit":["health_check_unit","health check unit"],
        "is_employment_verification_required":["is_employment_verification_required","is employment verification required"],
        "employment_verification_interval":["employment_verification_interval","employment verification interval"],
        "employment_verification_unit":["employment_verification_unit","employment verification unit"],
        "is_continued_business_need_required":["is_continued_business_need_required","is continued business need required"],
        "continued_business_need_interval":["continued_business_need_interval","continued business need interval"],
        "continued_business_need_unit":["continued_business_need_unit","continued business need unit"],
    }
    def resolve_template_field(canonical):
        for alias in policy_aliases.get(canonical,[canonical]):
            target=header_lookup.get(hkey(alias))
            if target:return target
        return None

    progress("Loading Flag Intelligence evidence",17)
    learner=FlagLearning();report_hash=learner.ingest(is_report)
    isdf,is_meta=read_tabular(is_report,True);flag_info=[]
    for h in headers:
        source_col=next((c for c in isdf.columns if hkey(c)==hkey(h)),None)
        if source_col:
            values={clean(x).upper() for x in isdf[source_col] if clean(x)}
            if values and values.issubset({"Y","N"}):
                priority="Priority 1" if meta is not None and clean(meta.get(h,"")).lower()=="mandatory" else "Priority 2"
                flag_info.append((h,priority))

    missing_fqdn_count=sum(1 for candidate,_ in eligible if not valid_fqdn(candidate["Fully qualified domain name"]))
    ...
    rows=[];policy_audit=[];learning_audit=[];ip_audit=[];fq_excluded=[];total=max(len(eligible),1)
    for pos,(r,d) in enumerate(eligible,1):
        ...
        if not fq and include_missing_fqdn:fq=normalize_hostname(r["Configuration Item"]).lower()+".nw.wpp.net"
        if not fq:
            z=r.to_dict();z["Exclusion Reason"]="MISSING VALID FQDN - USER SELECTED EXCLUDE";fq_excluded.append(z);continue
        ip,msg,more=normalize_ipv4(r["IP Address"])
        parent="Appliance" if pkey(r.get("Class","")) in {"wirelessaccesspoint","interfacecard"} else ("Network" if source_type=="network" else "OS")
        values={"hostname":normalize_hostname(r["Configuration Item"]),"fully_qualified_hostname":fq,"os_parent_serial_number":d.get("os_parent_serial_number",r["Serial number"]),"os_parent_opaque_id":d.get("os_parent_opaque_id",""),"os_parent_manufacturer":d.get("os_parent_manufacturer",""),"os_parent_hardware_type":d.get("os_parent_hardware_type",""),"os_parent_hardware_model":d.get("os_parent_hardware_model",""),"os_parent_category":d.get("os_parent_category",""),"os_lifecycle_status":"Production","parent_system_type":parent,"operating_system_provider":r["Required operating_system_provider"],"operating_system_name":r["Required operating_system_name"],"operating_system_version":r["Required operating_system_version"],"ip_address":ip}
        row={h:"" for h in headers};lookup={hkey(k):v for k,v in values.items()}
        for h in headers:
            if hkey(h) in lookup:row[h]=lookup[hkey(h)]
        context={"source_type":source_type,"subcategory":"Network" if source_type=="network" else ("Windows" if "windows" in clean(r["Required operating_system_name"]).lower() else "Server"),"parent_system_type":parent,"image_purpose":r.get("Class",""),"os_name":r["Required operating_system_name"],"os_version":r["Required operating_system_version"],"class":r.get("Class",""),"category":r.get("category",""),"attribute_field_value":r.get("Attribute Field Value",r.get("Attributes",""))}
        policy_values={};applied=apply_policy(policy_values,context)
        for canonical,value in policy_values.items():
            target=resolve_template_field(canonical)
            if target:row[target]=value
        for audit in applied:
            target=resolve_template_field(audit["Field"]);policy_audit.append({"CMDB Hostname":r["Configuration Item"],"CMDB Serial Number":r["Serial number"],**audit,"Template Field":target or "","Template Field Found":"Y" if target else "N"})
        ...
        for field,priority in flag_info:
            ...
            value,agreement,support,status,level=learner.predict(field,priority,cohort);applied_value="N"
            if status=="Auto-prefill" and value:row[field]=value;applied_value="Y"
            learning_audit.append({"CMDB Hostname":r["Configuration Item"],"CMDB Serial Number":r["Serial number"],"Field":field,"Priority":priority,"Proposed Value":value,"Agreement":agreement,"Support":support,"Decision":status,"Cohort Level":level,"Applied":applied_value})
        refs=[r["Configuration Item"],r["Serial number"],r["Life Cycle Stage"],r["Action"],r["Candidate Set ID"],"Y" if d["Recommended Action"]=="LOAD CATEGORY AND OS" else "N",d["Parent Dependency Status"],d["Catalog Match Status"]]
        rows.append(dict(zip(extras,refs),**row));ip_audit.append({"CMDB Hostname":r["Configuration Item"],"CMDB Serial Number":r["Serial number"],"Original IP":r["IP Address"],"Selected IP":ip,"Additional IPs":more,"Message":msg})
    learning_anomalies=learner.anomalies(report_hash);learner.close()
    metadata=["Reference"]*len(extras)+([clean(meta[h]) for h in headers] if meta is not None else [""]*len(headers));final=pd.concat([pd.DataFrame([metadata],columns=extras+headers),pd.DataFrame(rows,columns=extras+headers)],ignore_index=True)
    set_output_progress(progress);load_path=next_versioned_path(outdir,label+" Enriched OS Bulk Load",fmt)
    ...
    validation=write_workbook(next_versioned_path(outdir,label+" Enriched OS Bulk Load Validation","xlsx"),{"Workflow Summary":pd.DataFrame([("Reconciliation Load To IS candidates",len(candidates)),("Category Load required",sum(1 for _,d in eligible if d["Recommended Action"]=="LOAD CATEGORY AND OS")),("OS load rows generated",len(rows)),("Missing FQDN excluded",len(fq_excluded)),("Policy fields applied",sum(1 for x in policy_audit if x["Template Field Found"]=="Y"))],columns=["Metric","Value"]),"Policy Intelligence":pd.DataFrame(policy_audit),"Flag Intelligence":pd.DataFrame(learning_audit),"IP Validation":pd.DataFrame(ip_audit),"Missing FQDN Excluded":pd.DataFrame(fq_excluded),"Parent Dependency Validation":decisions,"Candidate Consistency":pd.DataFrame(consistency),"IS Learning Anomalies":learning_anomalies})
