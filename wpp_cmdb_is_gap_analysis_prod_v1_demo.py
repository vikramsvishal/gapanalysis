"""WPP Enterprise CMDB and Inventory Services Gap Analysis Prod V1 Demo
...
APP_NAME = "WPP Enterprise CMDB and Inventory Services Gap Analysis Prod V1 Demo"
APP_DIR = Path(__file__).resolve().parent
REGISTRY_PATH = APP_DIR / "prod_v1_demo_resource_registry.json"
LEARNING_DB = APP_DIR / "prod_v1_demo_flag_intelligence.sqlite"
...
def run_hardware_governance(domain,cmdb_path,category_path,catalog_path,outdir,progress=lambda m,p:None,os_reconciliation=None,is_os_path=None):
    ...
    p=next_versioned_path(outdir,label+" Category Load","xlsx");metadata=["Reference"]*len(prefix)+[""]*len(catdf.columns);pd.DataFrame([metadata]+loadrows,columns=prefix+list(catdf.columns)).to_excel(p,index=False);paths.append(p)
...
class App(tk.Tk):
    ...
    def reconcile_nw_click(self):
        ...
        if not out:return
        def task():
            self._load_field_mapping();r=reconcile_nw(read_tabular(self.paths["NW CMDB Report"]),read_tabular(self.paths["IS Operating System Report"]),read_tabular(self.paths["IS Product Catalog Operating System"]),self.fi,self.progress);self.results["network"]=r;p=next_versioned_path(out,"NW CMDB to IS Reconciliation V5.2","xlsx");write_workbook(p,{"Summary":nw_summary(r),"Reconciliation":r,"Load To IS":r[r.Action.str.contains("Load To IS",na=False)],"Retire From IS":r[r.Action.str.contains("Retire From IS",na=False)],"Inventory OS Update Required":r[r.Action.str.contains("Inventory OS update required",na=False)],"Catalogue Update Required":r[r.Action.str.contains("Catalogue update required",na=False)]});self.say("Created: "+p);self.after(0,lambda path=p:messagebox.showinfo("Complete",path))
        self.execute(task)
    def reconcile_server_click(self):
        ...
        if not out:return
        def task():
            self._load_field_mapping();r,outside,dups,historical=reconcile_server(read_tabular(self.paths["Server CMDB Report"]),read_tabular(self.paths["IS Operating System Report"]),read_tabular(self.paths["IS Product Catalog Operating System"]),self.fi,self.progress);self.results["server"]=r;p=next_versioned_path(out,"Server CMDB to IS Reconciliation Prod V1","xlsx");write_workbook(p,{"Summary":server_summary(r,outside,dups,historical),"Reconciliation":r,"Load To IS":r[r.Action.str.contains("Load To IS",na=False)] if not r.empty else r,"Retire From IS":r[r.Action.str.contains("Retire From IS",na=False)] if not r.empty else r,"Out of W-KYN Scope":outside,"Historical EOL Suppressed":historical,"CMDB Duplicate Review":dups});self.say("Created: "+p);self.after(0,lambda path=p:messagebox.showinfo("Complete",path))
        self.execute(task)
