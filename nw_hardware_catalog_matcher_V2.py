...
GUI inputs (exactly two):
  1. IS NW Catalog file (.xlsx, .xlsm, .xlsb, or .csv)
  2. NW CMDB report (.xlsx, .xlsm, .xlsb, or .csv)
...
class CatalogIndexes:
    """In-memory indexes built once for the complete catalog."""
...
def _read_xlsb(path: Path, expected_headers: Iterable[str]) -> TableData:
    """Read an Excel Binary Workbook using pyxlsb."""
    try:
        from pyxlsb import open_workbook
    except ImportError as exc:
        raise RuntimeError(
            "XLSB support requires pyxlsb. Install it with: pip install pyxlsb"
        ) from exc

    expected = {header_key(h) for h in expected_headers}
    best_headers: list[Any] | None = None
    best_rows: list[list[Any]] | None = None
    best_sheet = ""
    best_score = -1
    ...
    with open_workbook(str(path)) as workbook:
        for sheet_name in workbook.sheets:
            ...
            for row_number, values in enumerate(rows[:20]):
                score = len({header_key(v) for v in values if text(v)} & expected)
                if score > best_score:
                    best_score = score
                    best_headers = values
                    best_rows = rows[row_number + 1:]
                    best_sheet = sheet_name

    if best_headers is None or best_rows is None or best_score <= 0:
        raise ValueError(f"Expected columns were not found in {path.name}.")

    headers = [text(value) for value in best_headers]
    width = len(headers)
    normalized_rows = [(row + [None] * width)[:width] for row in best_rows]
    return TableData(headers, normalized_rows, best_sheet)
...
def read_table(path: Path, expected_headers: Iterable[str]) -> TableData:
    ...
    if suffix == ".xlsb":
        return _read_xlsb(path, expected_headers)
    raise ValueError(
        f"Unsupported file type for {path.name}. Use .xlsx, .xlsm, .xlsb, or .csv."
    )
...
def build_catalog_indexes(table: TableData) -> CatalogIndexes:
    """Load the catalog once and build all three lookup indexes once."""
    require_columns(table, ["hardware_type", "hardware_model", "manufacturer"], "IS NW Catalog")
    cols = table.columns
    records: list[CatalogRecord] = []
    exact_temp: dict[str, list[int]] = {}
    type_temp: dict[str, list[int]] = {}
    token_temp: dict[str, list[int]] = {}
    ...
    for source_row, row in enumerate(table.rows, start=2):
        ...
        if record.model_compact:
            exact_temp.setdefault(record.model_compact, []).append(record_id)
        if record.type_compact:
            type_temp.setdefault(record.type_compact, []).append(record_id)

        # Index both type and model fragments. A set prevents duplicate IDs per token.
        for token in set(record.type_tokens + record.model_tokens):
            token_temp.setdefault(token, []).append(record_id)
    ...
    return CatalogIndexes(
        ...
        exact_model_index={key: tuple(ids) for key, ids in exact_temp.items()},
        hardware_type_index={key: tuple(ids) for key, ids in type_temp.items()},
        partial_token_index={key: tuple(ids) for key, ids in token_temp.items()},
    )
...
def _records_for_ids(indexes: CatalogIndexes, record_ids: Iterable[int]) -> list[CatalogRecord]:
    return [indexes.records[record_id] for record_id in record_ids]


def match_model(cmdb_model: Any, indexes: CatalogIndexes) -> MatchResult:
    """Use indexes to obtain a small candidate set before applying match scoring."""
    raw = text(cmdb_model)
    key = compact(raw)
    if not key:
        return MatchResult(None, "BLANK_MODEL")

    exact_ids = indexes.exact_model_index.get(key, ())
    if exact_ids:
        exact = _records_for_ids(indexes, exact_ids)
        outputs = {
    ...
    candidate_ids: set[int] = set(indexes.hardware_type_index.get(key, ()))
    query_tokens = model_tokens(raw)
    for token in query_tokens:
        candidate_ids.update(indexes.hardware_type_index.get(compact(token), ()))
        candidate_ids.update(indexes.partial_token_index.get(token, ()))

    # Compound CMDB strings can contain a complete hardware_type plus another suffix.
    # Consult only indexed type keys, then add the matching IDs to the candidate set.
    for type_key, ids in indexes.hardware_type_index.items():
        if len(type_key) >= 3 and (type_key in key or key in type_key):
    ...
    candidates = [
        (fallback_score(raw, indexes.records[record_id]), indexes.records[record_id])
...
def create_output(catalog_path: Path, cmdb_path: Path, output_path: Path) -> dict[str, int]:
    ...
    catalog_indexes = build_catalog_indexes(catalog_table)
    ...
    try:
        ...
        try:
            ...
            for source_row in cmdb_table.rows:
                ...
                result = match_model(source_row[model_index], catalog_indexes)
...
def launch_gui() -> None:
    ...
    class MatcherGUI(tk.Tk):
        ...
        def file_types():
            return [
                ("Supported files", "*.xlsx *.xlsm *.xlsb *.csv"),
