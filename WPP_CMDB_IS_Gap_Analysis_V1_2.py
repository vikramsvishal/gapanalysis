"""WPP Enterprise CMDB and Inventory Services Gap Analysis V1.2
...
APP_NAME = "WPP Enterprise CMDB and Inventory Services Gap Analysis V1.2"
APP_DIR = Path(__file__).resolve().parent
REGISTRY_PATH = APP_DIR / "v1_2_resource_registry.json"
LEARNING_DB = APP_DIR / "v1_2_flag_intelligence.sqlite"
...
def _format_excel_workbook(path: str) -> None:
    ...
    for worksheet in workbook.worksheets:
        ...
        if worksheet.title.lower() == "summary" and worksheet.max_column >= 2:
            metric_column = next((i for i in range(1, worksheet.max_column + 1) if pkey(worksheet.cell(1, i).value) == "metric"), None)
            if metric_column:
                ...
                for row_index in range(2, worksheet.max_row + 1):
                    ...
                    if "loadtoisrecords" in metric: worksheet.cell(row_index, metric_column).fill = load_fill
                    elif "retirefromisrecords" in metric: worksheet.cell(row_index, metric_column).fill = retire_fill
...
def reconcile_nw(cm,isr,cat,fi,progress=lambda m,p:None):
    ...
    parsed.columns=["Parsed Firmware Name","Parsed Firmware Version","Parsed Firmware Provider","Firmware Mapping Method"]
